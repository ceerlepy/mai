...
```js
// src/router.js — her sınıfın kendi cache ömrü var
case CLASS.STATIC:
...
case CLASS.SUBJ:
  return { ..., ttl: 0 };        // saklanmaz — zaten cevap üretilmiyor
```
...
SEMI sınıfında yerel ve web **aynı anda** başlatılıyor:
...
async function gatherEvidence(env, q, context, cls, plan) {
  ...
  if (plan.local) {
    tasks.push(lookupLocal(env, q)
      .then((r) => (r && !isStale(r.fresh, r.maxDays) ? { ...r, src: "yerel", prio: 0 } : null)));
  }
  if (plan.web) {
    const qq = addTimeContext(buildQuery(q, context), cls);
    tasks.push(search(env, qq).then((r) => (r ? { ...r, src: "web", prio: 1 } : null)));
  }

  const results = await Promise.all(tasks.map((p) => p.catch(() => null)));
  const ok = results.filter(Boolean).sort((a, b) => a.prio - b.prio);
  return ok[0] || null;      // prio 0 = yerel öncelikli
}
```

```
Seri olsaydı:  yerel (80ms) → yok → web (900ms)  = 980 ms
Paralel:       max(80, 900)                       = 900 ms
Yerel hit:     80 ms'de hazır, web sonucu atılır
```

`prio` alanı sıralamayı belirliyor: ikisi de sonuç dönerse yerel kazanır
(daha güvenilir kaynak, sen yüklemişsin).
